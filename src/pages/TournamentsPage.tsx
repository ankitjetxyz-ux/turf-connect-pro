...
interface Tournament {
  ...
  prize: string;
  image: string;
  status: "Registering" | "Coming Soon" | "Closed";
  spotsLeft: number;
  entryFee: string;
  format: string;
}
...
const TournamentsPage = () => {
  ...
  const handleJoinTournament = async (tournamentId: string) => {
    ...
    try {
      // Call tournament join API here
      // await joinTournament(parseInt(tournamentId));
      alert("Successfully joined the tournament!");
      setJoiningTournament(null);
    } catch (error: any) {
      setJoiningError(error.response?.data?.error || "Failed to join tournament");
    }
  };
