...
const TurfDetailPage = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      console.log("Initiating booking for slot:", selectedSlot);
      // 1. Create Order
      const { data } = await api.post("/bookings/create-and-order", {
        slot_id: selectedSlot
      });
      
      console.log("Order created:", data);
      const { order, booking_id, key_id } = data;
      ...
      const options = {
        ...
        name: turf?.name || "Turf Booking",
        description: `Booking for ${slotData.start_time}`,
        image: "https://example.com/your_logo", // Optional: Add logo
        order_id: order.id,
        handler: async function (response: any) {
            ...
            try {
               ...
               setTimeout(() => navigate("/player/dashboard"), 1000);
            } catch (err) {
               console.error("Verification failed", err);
               showToast({ title: "Payment Failed", description: "Payment verification failed", variant: "destructive" });
               setBookingLoading(false);
            }
        },
        prefill: {
            name: "Player", // TODO: Get actual user name
            contact: "",    // TODO: Get actual contact
      ...
      rzp.on("payment.failed", function (response: any) {
          ...
          showToast({ title: "Payment Failed", description: response.error.description || "Payment failed", variant: "destructive" });
          setBookingLoading(false);
      });
      rzp.open();
      ...
      showToast({
        title: "Error",
        description: err.response?.data?.error || err.message || "Failed to initiate booking",
        variant: "destructive"
      });
      setBookingLoading(false);
    }
    // Note: setBookingLoading(false) is handled in modal.ondismiss or payment.failed or catch
    // We remove the finally block to keep loading true while Razorpay is open
