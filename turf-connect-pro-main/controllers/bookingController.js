...
exports.createBookingAndOrder = async (req, res, next) => {
  try {
    ...
    // create booking as confirmed (to satisfy DB constraint and reserve slot)
    const { data: booking, error: bookingErr } = await supabase.from("bookings").insert([{ user_id, slot_id, status: "confirmed" }]).select().single();
    if (bookingErr) return next(bookingErr);

    // Reserve the slot immediately
    await supabase.from("slots").update({ is_available: false }).eq("id", slot_id);

    // create razorpay order
    const amount = Number(slot.price || 0);
    if (!global.razorpay && !process.env.RAZORPAY_KEY_ID) {
      // If Razorpay fails, we should technically revert the booking, but for now let's just error
      return res.status(500).json({ error: "Razorpay not configured" });
    }
    const rp = new Razorpay({ key_id: process.env.RAZORPAY_KEY_ID, key_secret: process.env.RAZORPAY_KEY_SECRET });
    const order = await rp.orders.create({ amount: Math.round(amount * 100), currency: "INR", receipt: String(booking.id), payment_capture: 1 });
...
exports.cancelBooking = async (req, res, next) => {
  try {
    ...
    console.log(`[cancelBooking] Request: user=${user_id}, booking=${booking_id}`);
    ...
    if (fetchErr || !booking) {
      console.warn(`[cancelBooking] Booking not found or not confirmed: ${fetchErr?.message || "No match"}`);
